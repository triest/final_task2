   ...
   //     dump($girl);
    ...
    if($request->has('change')) {
        ...
        if ($request->has('region')) {
            ...
            DB::table('girls')->where('id', $girl->id)->update(['region_id' => $city]);
    ...
    public function search(Request $request){
     ...
     //   die();
        ...
        public function adminToGirl(Request $request){
            ...
            if ($request->has('unbanned'))
                $banned=0;
            else{
                $banned=1;
            }

            $girl = Girl::select(['name', 'email', 'password','id','phone','description','enabled','payday','payed','login',
                'main_image','sex','meet','weight','height','age','country_id','region_id','city_id','banned'])
...
//            dump($girl);
        ...
        public function usersList(Request $request){
            $users=User::select(['id','name', 'email','is_conferd','money','isAdmin','phone','phone_conferd','actice_code','akcept','smsResetCode'])
            ->simplePaginate(25);
      ...
      public function moneyHistory(Request $request){
          ...
          if($user->isAdmin==0 or $user==null )
          {
              return $this->index();
                ...
                //       $price_toTop=collect(DB::select('select price from servises where name=\'toTop\' '))->first(); //получили цену
          //$history=DB::select('SELECT * FROM `money_history` ORDER BY `date` DESC ')->paginate(50);  //получаем страны
